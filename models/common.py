...
# from models.test.BiFormer3 import BiLevelRoutingAttention
from models.att.LA import ResLA
...
class C3B(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)
        #yuanben 8-2
        self.m = BiLevelRoutingAttention(c_,7,8)
...
class C3LA(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)
        #yuanben 8-2
        # self.m = nn.Sequential(*(ResLA(c_) for _ in range(n)))
        self.m = ResLA(c_)

        # self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    # def forward(self, x):
    #     return self.cv3(torch.cat((self.m(self.la(self.cv1(x))), self.cv2(x)), 1))
from models.B.test import SSS
from models.B.test import ResST
class C3SSS(C3):
    # C3 module with TransformerBlock()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
    ...
    # def forward(self, x):
    #     return self.cv3(torch.cat((self.m(self.la(self.cv1(x))), self.cv2(x)), 1))
from models.B.cnn import ResCN
class C3CN(C3):
    # C3 module with TransformerBlock()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
...
class C3CAG(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(ContextAggregation(c_, 1) for _ in range(n)))
...
class CAM(nn.Module):
    def __init__(self, inc, fusion='weight'):
        ...
        self.conv2 = Conv(inc, inc, 3, 1, None, 1, 2)
        self.conv3 = Conv(inc, inc, 3, 1, None, 1, 3)
