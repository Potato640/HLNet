...
class LinAngularAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sparse_reg:
            ...
            mask = torch.exp(1 - 1/attn)
            sparse = mask * attn

        dconv_k = self.dconv(k)

        attn = self.qv_matmul(q.transpose(-2, -1), v)

        if self.sparse_reg:
            x = (
                self.sv_matmul(sparse,k)
                + 0.5 * k
                + 1.0 / math.pi * self.kqv_matmul(k, attn)
            )
        else:
            # x = 0.5 * k + 1.0 / math.pi * self.kqv_matmul(k, attn)
            x = 0.5 * k + b * self.kqv_matmul(k, attn)
        x = x / x.norm(dim=-1, keepdim=True)
        x += dconv_k

        # attn = x.softmax(dim=-1)
        # # print(attn)
        # mask = 2 - torch.exp(10*attn)
        # # print(mask)
        # x = mask * x
